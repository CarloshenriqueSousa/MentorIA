...
public class PlannerAgentService {
    ...
    public PlannerAgentResponse buildGuidance(UUID userId, String goal) {
        ...
        String fallback = """
                ...
                """.formatted(goal);
        String resolved = (content == null || content.isBlank() || "fallback".equalsIgnoreCase(response.model()))
                ? fallback
                : content;

        return PlannerAgentResponse.builder()
                .goal(goal)
                .recommendedCourses(resolved)
                .studyMethods(resolved)
                .weeklyPlanGuidance(resolved)
                .build();
    }
}
